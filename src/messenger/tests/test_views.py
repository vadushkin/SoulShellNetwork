...
class MessengerViewsTests(TestCase):
    def setUp(self):
    ...
    def test_user_messages(self):
        response = self.client.get(reverse("messenger:messages_list"))
    ...
    def test_user_conversation(self):
        response = self.client.get(
            reverse(
                "messenger:conversation_detail", kwargs={"username": self.user.username}
            )
        )
        assert response.status_code == 200
    ...
    def test_send_message_view(self):
        ...
        request = self.client.post(
            reverse("messenger:send_message"),
            {
                "to": "second_user",
    ...
    def test_message_reception_view(self):
        request = self.client.get(
            reverse("messenger:receive_message"),
            {
        ...
        assert "An answer message." == str(request.context["message"])
